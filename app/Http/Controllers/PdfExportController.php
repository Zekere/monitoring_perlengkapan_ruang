...
{
    /**
     * Opsi default dompdf agar berfungsi di hosting
     * Mengatasi error "Cannot resolve public path"
     */
    ...
    public function exportBarang()
    {
        ...
        $pdf = Pdf::setOptions($this->pdfOptions())
            ->loadView('pdf.barang', [
                'barang' => $barang,
    ...
    {
        $kategori = Kategori::withCount('items')->get();

        $pdf = Pdf::setOptions($this->pdfOptions())
            ->loadView('pdf.kategori', [
                'kategori' => $kategori,
    ...
    {
        $ruangan = Ruangan::withCount('items')->get();

        $pdf = Pdf::setOptions($this->pdfOptions())
            ->loadView('pdf.ruangan', [
                'ruangan' => $ruangan,
    ...
    {
        $query = Pengaduan::with(['user', 'item']);
    ...
    {
        $query = Pengecekan::with(['user', 'item']);
    ...
    public function exportRiwayatPerawatan(Request $request)
    {
        ...
        $pdf = Pdf::setOptions($this->pdfOptions())
            ->loadView('pdf.riwayat-perawatan', [
